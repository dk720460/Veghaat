    ...
    <div className="fixed inset-x-0 top-0 bottom-0 mx-auto max-w-[480px] z-[70] bg-[#F5F7FD] flex flex-col h-full animate-slide-in-right">
      ...
      <div className="flex-1 overflow-y-auto pb-32 no-scrollbar">
        {(isCancelled || isReturned) ? (
            <div className={`p-8 flex flex-col items-center justify-center text-center mt-10 mb-6 rounded-xl mx-4 ${isCancelled ? 'bg-red-50' : 'bg-orange-50'}`}>
                ...
                <p className="text-sm text-gray-600">
                    {isCancelled ? 'This order has been cancelled.' : 'Return request processed.'}
                </p>
            </div>
        ) : (
        <div className="w-full h-72 bg-gray-200 relative overflow-hidden border-b border-gray-200">
           <div className="absolute inset-0 opacity-40" style={{ backgroundImage: 'radial-gradient(#cbd5e1 2px, transparent 2px)', backgroundSize: '20px 20px', backgroundColor: '#e2e8f0' }}></div>
           <svg className="absolute top-0 left-0 w-full h-full" style={{zIndex: 1}}>
              <path d="M 50 80 C 120 80, 150 150, 300 180" fill="none" stroke="#d1d5db" strokeWidth="6" strokeLinecap="round" pathLength="100" />
              <path d="M 50 80 C 120 80, 150 150, 300 180" fill="none" stroke="#16a34a" strokeWidth="6" strokeLinecap="round" pathLength="100" strokeDasharray="100" strokeDashoffset={100 - progressPercent} className="transition-all duration-[2000ms] ease-in-out" />
           </svg>
           <div className="absolute top-[60px] left-[30px] bg-white p-2 rounded-full shadow-lg border-2 border-green-600 z-10">
              <div className="w-3 h-3 bg-green-600 rounded-full"></div>
           </div>
           <div className={`absolute top-[160px] left-[280px] p-2 rounded-full shadow-lg border-2 z-10 transition-colors duration-500 ${currentStatusNormalized === 'Delivered' ? 'bg-green-600 border-green-700' : 'bg-white border-red-500'}`}>
              <MapPin size={20} className={currentStatusNormalized === 'Delivered' ? 'text-white' : 'text-red-500'} fill="currentColor"/>
           </div>
           <div className="absolute transition-all duration-[2000ms] linear z-20" style={{ left: bikePosition.x - 24, top: bikePosition.y - 24 }}>
              <div className="relative">
                  <img src="https://cdn-icons-png.flaticon.com/512/7543/7543163.png" alt="Delivery Partner" className="w-12 h-12 drop-shadow-lg" />
           ...
           <div className="absolute bottom-4 left-4 bg-white/90 backdrop-blur px-3 py-1.5 rounded-lg shadow-lg flex items-center space-x-2 z-10 border border-green-100">
              ...
              <div>
                 ...
                 <p className="text-sm font-black text-gray-800 transition-all">{eta}</p>
              </div>
           </div>
        </div>
        )}
        ...
        <div className="bg-white p-4 mb-2 shadow-sm animate-fade-in border-b border-gray-100">
           <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                 ...
                 <div>
                    <h3 className="text-sm font-bold text-gray-900 leading-tight">{liveOrder.deliveryBoyName || 'Delivery Partner'}</h3>
                    <p className="text-xs text-gray-500">Your Delivery Partner</p>
                    <div className="flex items-center space-x-1 mt-1">
                       <span className="text-[10px] bg-green-100 text-green-700 px-1.5 rounded font-bold flex items-center">4.8 ★</span>
                       <span className="text-[10px] text-gray-400 flex items-center"><ShieldCheck size={10} className="mr-0.5 text-green-600"/> Vaccinated</span>
                    </div>
                 </div>
              </div>
              <div className="flex space-x-3">
                 {liveOrder.deliveryBoyPhone ? (
                     <a href={getPhoneLink(liveOrder.deliveryBoyPhone)} className="w-10 h-10 rounded-full bg-green-600 flex items-center justify-center text-white shadow-lg hover:bg-green-700 transition-colors animate-pulse">
           ...
           <div className="relative ml-3 space-y-0">
              {STATUS_STEPS.slice(1).map((step, idx) => { 
                ...
                <div key={idx} className="relative pl-8 pb-8 last:pb-0">
                   {!isLast && <div className={`absolute left-[6px] top-4 w-[2px] h-full ${isCompleted && currentStepIndex > stepIdx ? 'bg-green-600' : 'bg-gray-200'}`}></div>}
                   <div className={`absolute left-0 top-0 w-4 h-4 rounded-full border-2 flex items-center justify-center bg-white z-10 transition-colors duration-500 ${isCompleted ? 'border-green-600' : 'border-gray-300'}`}>
                      {isCompleted && <div className="w-2 h-2 bg-green-600 rounded-full"></div>}
                   </div>
                   <div className="-mt-1">
                      <h4 className={`text-sm font-bold transition-colors duration-300 ${isCompleted ? 'text-green-700' : 'text-gray-400'}`}>{step}</h4>
                      <p className="text-xs text-gray-500 mt-0.5">{isCurrent ? 'In Progress' : isCompleted ? 'Completed' : 'Pending'}</p>
                   </div>
                </div>
              )})}
           </div>
        ...
        <div className="bg-white shadow-sm mb-6">
           <button onClick={() => setIsItemsOpen(!isItemsOpen)} className="w-full flex items-center justify-between p-4 focus:outline-none hover:bg-gray-50 transition-colors">
              <div className="flex flex-col items-start">
                 <h3 className="text-sm font-bold text-gray-800">Items in this order</h3>
                 {!isItemsOpen && <span className="text-xs text-gray-500">{liveOrder.items.length} items • Total ₹{liveOrder.totalAmount}</span>}
              </div>
           ...
           {isItemsOpen && (
              <div className="border-t border-gray-100 p-4 bg-gray-50 animate-fade-in">
                 {liveOrder.items.map((item, idx) => (
                    <div key={idx} onClick={() => handleItemClick(item.productId)} className="flex justify-between items-center mb-3 last:mb-0 cursor-pointer hover:bg-gray-50 p-1 rounded-lg transition-colors">
                       <div className="flex items-center space-x-3">
